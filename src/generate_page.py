...
def generate_page(from_path, template_path, dest_path):
    ...
    html = template_file.replace("{{ Title }}", title).replace("{{ Content }}", from_html)
    ...
    for i in from_dir_ls:
        ...
        if os.path.isdir(path):
            ...
            if not os.path.exists(to_dir):
                os.mkdir(to_dir)
            generate_pages_recursive(path, template_path, to_dir)
        else:
            generate_page(path, template_path, os.path.join(dest_dir_path, i.replace(".md", ".html")))